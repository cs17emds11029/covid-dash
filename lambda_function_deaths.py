...
class Covid19:
    ...
    '''
    def transform_and_write_deaths_global(self,access_key,secret_access):
        # The format of the file is Province/State, Country/Region and a series of dates
        # transform the format into date, Province/State, Country/Region and cumcount
        self.covid_ts_deaths_global.drop(labels=['Lat','Long'],axis=1,inplace=True)
        self.covid_ts_deaths_global['Province/State'].fillna(self.covid_ts_deaths_global['Country/Region'],inplace=True)
        self.covid_ts_deaths_global = self.covid_ts_deaths_global.pivot_table(columns=['Province/State','Country/Region']).reset_index()
        self.covid_ts_deaths_global.columns = ['date','province_state','country_region','cumulative_count']
        
        # generate a new count column
        self.covid_ts_deaths_global.sort_values(by=['country_region','province_state','date'])
        self.covid_ts_deaths_global['new_count'] = self.covid_ts_deaths_global.groupby(['country_region','province_state'])['cumulative_count'].diff().fillna(0)
        
        #self.covid_ts_global.to_csv('D:/Projects/covid-dash/covid_ts_global.csv', index=False)
        self.write_to_s3("covid_ts_deaths_global.csv",
                         self.covid_ts_deaths_global,
                         access_key,
                         secret_access)
...
def lambda_handler(event, context):
    Covid19obj = Covid19()
    Covid19obj.transform_and_write_deaths_global(event['ACCESS_KEY'],
                                                 event['SECRET_ACCESS'])